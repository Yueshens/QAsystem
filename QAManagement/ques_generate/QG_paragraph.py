...
class Paragraph(object):
    ...
    def Phrase_type_title(self,dic,title):
        ...
        # 是正常的标题
            ...
            if 'subject' in dic.keys() and (len(dic['subject']) > 0):
                ...
                for arc in result:
                    if arc[1] == 'VOB':  # 词里面有动宾关系，可以提问如何xxx
                        VOB_flag = True
                        # 只要有直接退出
                        break
                    else:
    ...
    def Phrase_type_subtitle(self,dic,title):
        ...
        title_list = Paragraph.segmentor.segment(title)
                    ...
                    for subtitle in subtitles:
                        ...
                        if dic[list(subtitle.keys())[0].replace('title', 'content')].strip() != '':
                            ...
                            # 正常的小标题，都是单个词语
                                ...
                                # 小标题先分词
                                # subtitle_words = list(Paragraph.segmentor.segment(subtitle[list(subtitle.keys())[0]]))
                                subtitle_words = Paragraph.segmentor.segment(subtitle[list(subtitle.keys())[0]])
                                # 分词后看句法关系
                                result = self.ltp_request(subtitle_words)
    ...
    # 发送LTP请求的函数
    def ltp_request(self,words):

        postags = Paragraph.postagger.postag(words)
        arcs = Paragraph.parser.parse(words, postags)
        return arcs
